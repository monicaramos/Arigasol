...
Begin VB.Form frmDecGasPro 
   ...
   Begin VB.CommandButton cmdDuplicidades 
      Caption         =   "Duplicidades"
      Height          =   375
      Left            =   120
      TabIndex        =   5
      Top             =   2520
...
Private Sub cmdDuplicidades_Click()
    Dim respuesta As String
    Dim mens As String
    '-- Primero preguntamos la fecha
    respuesta = InputBox("Desde fecha:", "Recuperar duplicidades")
    If respuesta = "" Or (Not IsDate(respuesta)) Then
        MsgBox "Necesito una fecha válida"
        Exit Sub
    End If
    '-- ya tenemos la fecha
    Set gdgp = New GestorDeclaracionesGasoleoProf
    If gdgp.detectaDuplicidades(CDate(respuesta)) Then
        mens = "Se han detectado duplicidades no declaradas desde la fecha indicada." & vbCrLf & _
                "¿Desea declararlas ahora?"
        If MsgBox(mens, vbYesNo + vbQuestion + vbDefaultButton2) = vbYes Then
            If gdgp.corrigeDuplicidades(CDate(respuesta)) Then
                MsgBox "Se ha preparado una declaración con las duplicidades, se debe enviar manualmente"
            Else
                MsgBox "Se ha producido un error generando la declaracion"
            End If
        End If
    Else
        MsgBox "No se detectan duplicidades no declaradas"
    End If
End Sub
