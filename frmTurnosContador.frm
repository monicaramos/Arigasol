...
Begin VB.Form frmTurnosContador 
   ...
   ClientTop       =   330
   ClientWidth     =   10695
   Icon            =   "frmTurnosContador.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5925
   ...
   Begin VB.ComboBox Combo1 
      Appearance      =   0  'Flat
      Height          =   315
      ItemData        =   "frmTurnosContador.frx":000C
      Left            =   2400
      List            =   "frmTurnosContador.frx":000E
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmTurnosContador.frx":0010
      Height          =   4410
      Left            =   120
      TabIndex        =   16
      Top             =   540
   ...
   End
End
Attribute VB_Name = "frmTurnosContador"
...
'    PonerIndicador lblIndicador, Modo
    ...
    btnBuscar(2).visible = False 'Not b
    ...
    Next i
    BloquearBtn btnBuscar(2), (Modo <> 1) 'boton de la fecha
    Me.Combo1.Enabled = (Modo = 1)
...
'    Next i
    ...
    CadenaConsulta = CadenaConsulta & "sartic.nomartic, "
    CadenaConsulta = CadenaConsulta & "sturno.containi, "
    CadenaConsulta = CadenaConsulta & "sturno.contafin "
    CadenaConsulta = CadenaConsulta & " FROM sturno, tiporegi, sartic "
    ...
    CadenaConsulta = CadenaConsulta & " AND   sturno.fechatur = " & DBSet(Fecha, "F")
    CadenaConsulta = CadenaConsulta & " AND   sturno.codturno = " & DBSet(Turno, "N")
    '************************************************************************
...
'    adodc1.ConnectionString = Conn
    ...
    End If
    '********************* canviar el ORDER BY *********************++
    sql = sql & " ORDER BY fechatur desc, codturno, numlinea "
    ...
    tots = tots & "N|||||;S|Combo1|C|T.Registro|1125|;"
    tots = tots & "S|txtAux(3)|T|Tanque|900|;S|txtAux(4)|T|Manguera|950|;S|txtAux(5)|T|Código|850|;"
    tots = tots & "S|txtAux2(0)|T|Artículo|1750|;S|txtAux(6)|T|C.Inicial|1250|;S|txtAux(7)|T|C.Final|1250|;"
'    For i = 1 To 8
'        tots = tots & "N||||0|;"
'    Next i
...
Dim Indicador As String
    ...
    If DatosOk Then
        ...
        If ActualizarExistencia(txtAux(6).Text, txtAux(7).Text) Then
            TerminaBloquear
...
Private Function ActualizarExistencia(ContaIni As Double, ContaFin As Double) As Boolean
...
Dim ADonde As String
    ...
    Conn.BeginTrans
    'Actualizar la Tabla: sturno con la cantidad introducida
    '-------------------------------------------------------
    ADonde = "Modificando Contadores de Turnos."
    sql = "UPDATE sturno Set containi = " & DBSet(ContaIni, "N") & ",contafin = " & DBSet(ContaFin, "N")
    sql = sql & " WHERE fechatur =" & DBSet(adodc1.Recordset!fechatur, "F")
    sql = sql & " and codturno = " & adodc1.Recordset!codTurno
...
EActualizar:
    If Err.Number <> 0 Then
        'Hay error , almacenamos y salimos
         sql = "Actualizando Contadores de Turnos." & vbCrLf & "--------------------------------------------" & vbCrLf
         sql = sql & ADonde
...
' *************************************
    ...
    If Not b Then Exit Function
     ...
     If b And (Modo = 4) Then
        '*************************************************************************************
        If CDbl(txtAux(6).Text) > CDbl(txtAux(7).Text) Then
            MsgBox "El Contador Final no puede ser inferior al inicial. Revise.", vbExclamation
            PonerFoco txtAux(7)
            b = False
        End If
     End If
...
'    PonerContRegIndicador
    ...
    If Not adodc1.Recordset.EOF And gridCargado And Modo = 4 Then
       CargaTxtAux True, True
    End If
...
Dim alto As Single
    ...
    If Not visible Then
        ...
        txtAux(6).Top = 290
        txtAux(6).visible = visible
        txtAux(7).Top = 290
        txtAux(7).visible = visible
    Else
        DeseleccionaGrid Me.DataGrid1
        If limpiar Then 'Vaciar los textBox (Vamos a Insertar)
                txtAux(6).Text = DBLet(adodc1.Recordset!ContaIni, "N")
                txtAux(6).Text = Format(txtAux(6).Text, "##,###,##0.00")
                txtAux(6).Locked = False
                txtAux(7).Text = DBLet(adodc1.Recordset!ContaFin, "N")
                txtAux(7).Text = Format(txtAux(7).Text, "##,###,##0.00")
                txtAux(7).Locked = False
        End If

        If DataGrid1.Row < 0 Then
            alto = DataGrid1.Top + 220
        Else
            alto = DataGrid1.Top + DataGrid1.RowTop(DataGrid1.Row) + 5
        End If
        
        'Fijamos altura y posición Top
        '-------------------------------
        txtAux(6).Top = alto
        txtAux(6).Height = DataGrid1.RowHeight
        txtAux(7).Top = alto
        txtAux(7).Height = DataGrid1.RowHeight
        
        'Fijamos anchura y posicion Left
        '--------------------------------
        txtAux(6).Left = DataGrid1.Columns(9).Left + 130 'contador ini
        txtAux(6).Width = DataGrid1.Columns(9).Width - 70
        txtAux(7).Left = DataGrid1.Columns(10).Left + 130 'contador ini
        txtAux(7).Width = DataGrid1.Columns(10).Width - 70
    ...
    If visible Then
        txtAux(7).SelStart = 0
        txtAux(7).SelLength = Len(txtAux(6).Text)
    End If
